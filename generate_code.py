#!/usr/bin/env python3
"""Generate Python programs using an instruct model for self-compression experiments.

Usage:
    .venv/bin/python3 generate_code.py --model <path> --slug <name> --count 5
"""
...
def generate_programs(model_path, slug, count, bf16=False):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_path,
        torch_dtype=torch.bfloat16 if bf16 else (torch.float16 if device == "cuda" else "auto"),
        device_map=device,
    ...
    for i in range(min(count, len(PROMPTS))):
        ...
        messages = [
            {"role": "user", "content": prompt + "\n\nRespond with ONLY the Python code, no explanations or markdown fences."}
        ]
        ...
        # Save
        filename = f"generated_{slug}_{i+1}.py.txt"
...
def main():
    parser = argparse.ArgumentParser(description="Generate Python code with instruct model")
    parser.add_argument("--model", type=str, required=True, help="Path to instruct model")
    parser.add_argument("--slug", type=str, required=True, help="Short name for file naming")
    parser.add_argument("--count", type=int, default=5, help="Number of programs to generate")
    parser.add_argument("--bf16", action="store_true", help="Use bfloat16")
    args = parser.parse_args()

    files = generate_programs(args.model, args.slug, args.count, args.bf16)
